  ...
  Promise.all(promiseArray)
    .then((values) => {
      const svgDataURL = values.map((item: Uint8Array) => {
        if (type === "dataurl") {
          return `${svgToDataURL(String.fromCharCode.apply(null, item))}`;
        }
        if (type === "base64") {
          return `${svgToBase64(String.fromCharCode.apply(null, item))}`;
        }
      });
      figma.ui.postMessage(
        { pluginMessage: { type: "copy", text: svgDataURL.join("\n") } },
        { origin: "*" }
      );
    })
    .catch((err: string) => {
